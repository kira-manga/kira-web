...
export const alt = siteConfig.socialCard.alt;
export const size = { width: 1200, height: 630 };
export const contentType = 'image/png';
export const dynamic = 'force-static';

export default function OpenGraphImage() {
  const copy = siteConfig.socialCard;
  ...
  return new ImageResponse(
    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '74px 86px', background: '#0e1014', color: '#f3f4f7', fontFamily: 'Arial, sans-serif' }}>
      <div style={{ display: 'flex', flexDirection: 'column', maxWidth: 760 }}>
        <div style={{ color: '#ff5b6e', fontSize: 24, fontWeight: 700, letterSpacing: 4, textTransform: 'uppercase' }}>{copy.eyebrow}</div>
        <div style={{ marginTop: 28, display: 'flex', flexDirection: 'column', fontSize: 76, lineHeight: 1.03, fontWeight: 800, letterSpacing: -4 }}>{copy.title}<span style={{ color: '#ff6370' }}>{copy.accent}</span></div>
        <div style={{ marginTop: 30, color: '#9aa1af', fontSize: 28 }}>{copy.description}</div>
      </div>
